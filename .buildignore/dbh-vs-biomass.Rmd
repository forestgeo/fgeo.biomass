...
A few points over `3e4` [kg] seem relatively too high. Remember that we still don't support some features (e.g. dbh-specific equations) that likely affect the magnitude of `biomass`.
...
For a better visualization let's remove those values for now, and plot `dbh` versus `biomass` again, this time by species.
