...
test_that("winner rows depend on the order of the list-elements", {
  ...
  expect_equal(reduce_complement(prio), combo)
  expect_equal(reduce_complement(prio, c(1, 2, 3)), combo)
  ...
  expect_equal(reduce_complement(prio, rev(order)), combo)
  expect_equal(reduce_complement(prio, c(3, 2, 1)), combo)
})
...
test_that("errs with informative message", {
  ...
  expect_warning(reduce_complement(prio), "Adding `rowid` to")
})
