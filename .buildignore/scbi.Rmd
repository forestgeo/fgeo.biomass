...
```{r}
# Subset the census data for speed
census <- fgeo.biomass::scbi_tree1 %>% sample_n(1000)
census
...
```{r, cache=TRUE}
census2 <- census %>% filter(!is.na(dbh))
naniar::vis_miss(census2)
```
...
```{r}
census_species <- census2 %>% add_species(species, site = "scbi")
census_species
```
...
```{r}
# Degrade the species table to force missmatches
species2 <- species %>% sample_frac(0.5)

census2_species2 <- census2 %>% add_species(species2, site = "scbi")
census2_species2 %>% filter(is.na(sp))
```

But my has not that problem. My next step is then to find default equations in the allodb package. I can to this with `dplyr::left_join()`.

```{r}
with_equations <- left_join(census_species, default_equations)
with_equations
```

I notice that some of the censused species have no match in allodb.

```{r}
with_equations %>% filter(is.na(equation_id))
```

I also notice that, for some trees match multiple equations. This is expected as some equations apply not to a whole tree but to part of a tree. The biomass of such trees will be the sum of the biomass of each component (more on this later).

```{r}
with_equations %>% 
  add_count(rowid) %>%
  filter(n > 1) %>% 
  select(rowid, sp, dbh, equation_id, anatomic_relevance, n)
```

But I suspect a mistake: I don't think two rows in a same rowid should be at the "whole tree" level.
...
with_equations %>% 
  ...
  filter(n > 1) %>% 
  select(rowid, sp, dbh, equation_id, anatomic_relevance, n) %>% 
  filter(grepl("whole tree", anatomic_relevance))
```
