...
test_that("errs with informative message", {
  ...
  expect_error(complement(winner, loser), msg)
...
test_that("the winner dataframe is the first one of the list", {
  ...
  expect_equal(purrr::reduce(prio, complement), prio1)

  prio <- list(prio2, prio1, prio3)
  expect_equal(purrr::reduce(prio, complement), prio2)

  prio <- list(prio3, prio2, prio1)
  expect_equal(purrr::reduce(prio, complement), prio3)
})
...
test_that("the winner can be determined by reordering the list", {
  ...
  expect_equal(purrr::reduce(prio[order], complement), prio[[3]])
  expect_equal(purrr::reduce(prio[rev(order)], complement), prio[[2]])
})
